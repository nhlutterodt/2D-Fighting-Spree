...
const MatchPreview = ({ config, p1, p2, stage, onExit }) => {
  ...
  useEffect(() => {
    console.log('🎮 MatchPreview mounted - setting up game loop');
    let anim = 0;
    ...
    const onKeyDown = (e) => {
      keyPressCount++;
      const k = e.key;
      console.log(`⌨️ KeyDown #${keyPressCount}: "${k}" (code: ${e.code})`);
      
      if (k === ' ' || e.code === 'Space') {
        keys['ArrowUp'] = true;
        console.log('  → Mapped to Jump (ArrowUp)');
        e.preventDefault();
        return;
      }
      if (k === 'ArrowUp' || k === 'w' || k === 'W') {
        console.log('  → Blocked (use Space for jump)');
        e.preventDefault();
        return;
      }
      if (k === 'ArrowLeft' || k === 'ArrowRight') {
        keys[k] = true;
        console.log(`  → Registered: ${k} = true`);
        e.preventDefault();
        return;
      }
      if (k === 'Shift' || k === 'z' || k === 'Z') {
    ...
    const stepSim = (dt) => {
      ...
      // Initialize lastGrounded on first frame
      if (firstFrame) {
        f1.lastGrounded = tNow;
        f2.lastGrounded = tNow;
        firstFrame = false;
        console.log('🔧 First frame init: lastGrounded set to', tNow.toFixed(3));
      }
      ...
      if (frameCount % 60 === 0) {
        ...
        if (activeKeys.length > 0) {
          console.log(`🎮 Frame ${frameCount} - Active keys:`, activeKeys);
          console.log(`   Input state: left=${left}, right=${right}, jump=${jumpHeld}, dash=${dashHeld}, atk=${atkHeld}`);
          console.log(`   Fighter: x=${Math.round(f1.x)}, y=${Math.round(f1.y)}, vx=${Math.round(f1.vx)}, vy=${Math.round(f1.vy)}`);
        }
      }
      ...
      if (jumpPressed) {
        ...
        console.log('    f1.grounded:', f1.grounded);
        console.log('    f1.y:', Math.round(f1.y), '(floor:', floor, ')');
        console.log('    f1.vy:', Math.round(f1.vy));
        console.log('    f1.jumpsLeft:', f1.jumpsLeft, '/ MAX_JUMPS:', MAX_JUMPS);
        console.log('    lastGrounded:', (tNow - f1.lastGrounded).toFixed(3), 's ago (COYOTE:', COYOTE_TIME, ')');
        f1.jumpBufferedAt = tNow;
      }
      ...
      const tryJump = (force = false) => {
        console.log('  ✅ EXECUTING JUMP! vy:', f1.vy, '→', -JUMP_VEL);
        f1.vy = -JUMP_VEL;
        f1.grounded = false;
      ...
      if ((canGroundJump && (buffered || jumpPressed)) && f1.jumpsLeft === MAX_JUMPS) {
        console.log('  → Ground jump condition met!');
        tryJump();
      } else if (jumpPressed && f1.jumpsLeft > 0 && !f1.grounded && !f1.wallSlide) {
        console.log('  → Air jump condition met!');
        tryJump();
      } else if (jumpPressed) {
        console.log('  ❌ Jump conditions NOT met:');
        console.log('    canGroundJump:', canGroundJump, 'buffered:', buffered, 'jumpsLeft:', f1.jumpsLeft);
        console.log('    grounded:', f1.grounded, 'wallSlide:', f1.wallSlide);
      }
  ...
  return (
    ...
    >
      ...
      <Card>
        <div className="mb-2 p-2 bg-yellow-500/20 rounded text-xs font-mono">
          🐛 DEBUG MODE - Check browser console (F12) for detailed input logs
        </div>
        <canvas
          ref={canvasRef}
